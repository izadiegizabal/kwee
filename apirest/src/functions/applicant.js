...
async function createApplicant(req, res, next, db, regUser) {
    ...
    try {
        ...
        if ( regUser ) {
            ...
            return db.sequelize.transaction(transaction => {
                return db.users.update(user, {transaction: transaction})
                .then(async user => {
                    uservar = user;
...
async function newApplicant(body, user, next, transaction, db) {
    try {
        let applicant = {};
